...
if (!apiKey) {
  console.warn('âš ï¸ Anthropic API key not configured. AI features will be limited.');
}
  ...
  if (lowerQ.includes('decision') || lowerQ.includes('decided')) {
    ...
    if (decisions.length > 0) {
      return `Based on the transcript, potential decisions include:\n\n${
        decisions.slice(0, 3).map(d => `â€¢ ${d.speaker.name}: "${d.text}"`).join('\n')
      }`;
    }
  ...
  if (lowerQ.includes('action') || lowerQ.includes('task') || lowerQ.includes('todo')) {
    ...
    if (actions.length > 0) {
      return `Potential action items:\n\n${
        actions.slice(0, 4).map(a => `â€¢ ${a.speaker.name}: ${a.text}`).join('\n')
      }`;
    }
