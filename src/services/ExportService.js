...
import { downloadFile, exportToCSV, exportToICS, formatDate, formatDuration, showToast } from '../utils/helpers.js';
...
class ExportService {
  ...
  generatePrintableHTML(meeting, minutes) {
    ...
    return `
      ...
      <body>
        ...
        <div class="footer">
          Generated by MeetingMind Enterprise • ${formatDate(new Date(), 'datetime')}
        </div>
      </body>
      </html>
    `;
  ...
  async copyToClipboard(meeting, minutes) {
    ...
    if (minutes.key_points?.length > 0) {
      text += `KEY POINTS\n`;
      minutes.key_points.forEach(point => text += `• ${point}\n`);
      text += '\n';
    }
